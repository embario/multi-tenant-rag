...
class Document(Base):
    ...
    source_type: Mapped[str] = mapped_column(String(32), nullable=False)  # "upload" | "url" etc.
...
class QueryLog(Base):
    ...
    cost_usd_micros: Mapped[int] = mapped_column(Integer, nullable=False, default=0)  # store micros to avoid float
...
class EvalRun(Base):
    ...
    query_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("query_logs.id", ondelete="CASCADE"), nullable=False)
    metric: Mapped[str] = mapped_column(String(64), nullable=False)
    score: Mapped[int] = mapped_column(Integer, nullable=False)  # keep simple (0-100) for v1
